...
MovableText::MovableText(const String &name, const String &caption, const String &fontName, Real charHeight, const ColourValue &color)
  : mpCam(NULL), mpWin(NULL), mpFont(NULL), mName(name), mCaption(caption), mFontName(fontName), mCharHeight(charHeight), mColor(color), 
    mType("MovableText"), mTimeUntilNextToggle(0), mSpaceWidth(0), mUpdateColors(true), mOnTop(false), mHorizontalAlignment(H_LEFT), 
    mVerticalAlignment(V_BELOW), mGlobalTranslation(0.0), mLocalTranslation(0.0) {
...
void MovableText::_setupGeometry() {
    printf("MovableText::_setupGeometry()\n");
...
void MovableText::_updateColors(void) {
    printf("MovableText::_updateColors(void)\n");
    assert(mpFont);
    assert(!mpMaterial.isNull());
...
//    printf("MovableText::_updateRenderQueue(RenderQueue* queue)\n");
